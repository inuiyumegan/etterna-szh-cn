...
local letter =
	LoadFont("Common Normal") ..
	{
		InitCommand = function(self)
			self:zoom(0.8):shadowlength(0)
		end,
		PulseCommand = function(self)
			self:finishtweening()
			local z = self:GetZoomX()
			self:accelerate(0.15):zoom(1.2):decelerate(0.15):zoom(z)
		end
	}
...
for pn in ivalues(PlayerNumber) do
	children[#children + 1] =
		LoadActor(THEME:GetPathG("_frame", "1D"), {4 / 10, 2 / 10, 4 / 10}, LoadActor(CursorFiles[pn])) ..
		{
			Name = CursorFiles[pn],
			...
			OnCommand = function(self)
				self:zoom(0):rotationz(-360 * 2):sleep(0.55):decelerate(0.5):zoom(1):rotationz(0)
			end,
			OffCommand = function(self)
				self:sleep(0.3):queuecommand("TweenOff")
			end,
...
return Def.ActorFrame {
		...
		if param.Name == "NextRow" or param.Name == "PrevRow" then
			...
			for l in ivalues(Letters) do
				if iRow == Keys[l].Row then
					...
					local fDist = distance(PlayerX[pn], PlayerY[pn], Text:GetX(), Text:GetY())
	...
	SelectKeyMessageCommand = function(self, param)
		...
		c.Cursors[pn]:playcommand(
			"SetSize",
			{
				Width = Keys[key].Width + 6,
				tween = function(self)
					self:stoptweening():linear(0.10)
				end
			}
		)
	...
	OnCommand = function(self, param)
		for key in ivalues(Letters) do
			...
			local f = function(self)
				self:diffusealpha(0):zoom(0):sleep(0.25 + (fDist / 400)):decelerate(0.5):diffusealpha(1):zoom(0.8)
			end
			f(Keys[key].Text)
		end
	end,
	OffCommand = function(self, param)
		for key in ivalues(Letters) do
			local fDist = distance(Keys[key].Text:GetX(), Keys[key].Text:GetY(), 0, 100)
			local f = function(self)
				self:sleep(0.0 + (fDist / 400)):decelerate(0.5):diffusealpha(0):zoom(0)
			end
			f(Keys[key].Text)
		end
	end
